...
def starter(request):
    if request.method == "POST":
        ...
        if ip:  
            print(ip)
            return redirect("stream:face", ip)
        else:
            messages.info(request, "Please input an ip address and server port number")
    return render(request, "starter.html")



# detect faces in a video and saves it .
def get_face_frame(ip):
...
@gzip.gzip_page
def capture_face(request, ip):
    return StreamingHttpResponse(get_face_frame(ip), content_type='multipart/x-mixed-replace; boundary=frame')
